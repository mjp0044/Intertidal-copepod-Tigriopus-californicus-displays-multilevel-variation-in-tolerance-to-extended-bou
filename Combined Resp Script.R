...
#If you accidentally recorded every 15 seconds, run this first before anything else
#dat = dat[seq(1, nrow(dat), 20), ] #Only keeps every 20th row which corresponds to every 300 seconds

#If you recorded every 30 seconds, run this first before anything else
#dat = dat[seq(1, nrow(dat), 20), ] #Only keeps every 20th row which corresponds to every 300 seconds
...
#Export graph (run three lines as single block of code, change name to whatever you want to call it)
  jpeg(file="03-21-23 1113 full.jpg", units="in", width=9, height=6, res=500)
  p1
  dev.off()
...
#Export graph (run three lines as single block of code, change name to whatever you want to call it)
  jpeg(file="03-21-23 1113 UNIQUE.jpg", units="in", width=9, height=6, res=500)
  p2
  dev.off()
...
#pdf(file='03-21-23 1113 plots.pdf')
...
#Changed cencoring parameters and saved other statistics data frames for comparison
  write.csv(calcs,"Plate_Resp_Statistics_03-21-23 1113.csv", row.names = FALSE)
